...
let random_nat_in ?rng min max : unit -> Nat.nat =
  ...
  if cmp > 0 then invalid_arg "random_nat_in";
  if cmp = 0 then fun () -> Bn.copy min else
  let n = Bn.sub max min in
  let numbits = Bn.num_bits n in
  let rec loop () =
    let v = Bn.random_nat ?rng numbits in	(* {0,…,2^{numbits}-1} *)
    if Bn.compare v n > 0 then loop()
    else	(* 0 ≤ v ≤ n = max - min *)
      Bn.add min v	(* {min,…,max} *)
  in
  loop
...
let new_QR_n_member ?rng ~base m =
  let pick_exponent : unit -> Nat.nat =
    Numutil.random_nat_in ?rng Bn.one m.p'q'
  in
  fun () ->
